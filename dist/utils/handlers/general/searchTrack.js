...
export async function searchTrack(client, query, source = "youtube") {
    ...
    if (queryData.isURL) {
            ...
            case "youtube":
                {
                    switch(queryData.type){
                        case "track":
                            {
                                const track = await youtube.getVideo(/youtu\.be/g.exec(url.hostname) ? url.pathname.replace("/", "") : url.searchParams.get("v") ?? '');
                                if (track) {
                                    result.items = [
                                        {
                                            duration: track.isLiveContent ? 0 : track.duration,
                                            id: track.id,
                        ...
                        case "playlist":
                            {
                                ...
                                let temp = null;
                                if (playlist) {
                                    const tracks = await Promise.all((playlist instanceof Playlist ? playlist.videos.items : playlist.videos).map((track)=>({
        ...
        if (source === "soundcloud") {
            ...
            const tracks = await Promise.all(searchRes.collection.map((track)=>({
                    duration: track.full_duration,
                    id: track.id.toString(),
                    thumbnail: track.artwork_url,
                    title: track.title,
            ...
            const tracks = await Promise.all(searchRes.items.map((track)=>({
                    duration: track.duration ?? 0,
                    id: track.id,
                    thumbnail: track.thumbnails.sort((a, b)=>b.height * b.width - a.height * a.width)[0].url,
                    title: track.title,
